...
class PositionUtils {
  ...
  static Position fromAlgebraic(String notation) {
    ...
    int col = notation.codeUnitAt(0) -
        'a'.codeUnitAt(0); // Convert 'a' to 0, 'b' to 1, etc.
  ...
  static String toAlgebraic(Position position) {
    String file = position
        .col; // Use Position's colLetter getter to convert column index to letter
    String rank =
        (8 - position.row).toString(); // Convert row index to chessboard rank

    return "$file$rank";
  }

  static int manhattanDistance(Position pos1, Position pos2) {
    int ans = ((pos1.row - pos2.row) +
            ((pos1.col.codeUnitAt(0) - 'a'.codeUnitAt(0)) -
                pos2.col.codeUnitAt(0) -
                'a'.codeUnitAt(0)))
        .toInt();
    return ans;
  }
  ...
  static bool isDiagonal(Position pos1, Position pos2) {
    return (pos1.row - pos2.row).abs() ==
        ((pos1.col.codeUnitAt(0) - 'a'.codeUnitAt(0)) -
                (pos2.col.codeUnitAt(0) - 'a'.codeUnitAt(0)))
            .abs();
  }
